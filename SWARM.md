...
1. **Bắt đầu session** → `node swarm.js history` rồi đọc `swarm/notes.md` và `swarm/workflow.md`
2. **Bắt đầu task** → `node swarm.js send "start | task: <mô tả> | branch: <branch>"`
3. **Tiến độ/milestone** → `node swarm.js send "progress | task: <mô tả> | status: <cập nhật>"`
4. **Hoàn thành task** → `node swarm.js send "done | task: <mô tả> | branch: <branch> | PR: <link>"`
5. **Bị block / cần input** → `node swarm.js send "blocked | task: <mô tả> | need: <yêu cầu>"`
6. **Insight quan trọng** → `node swarm.js notes "<ghi chú>"`

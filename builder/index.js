...
await FS.write([...paths.build.statics, "tags", "meta.json"], {
...
for (let page = 1; page <= Math.max(1, totalTagPages); page++) {
    ...
    await FS.write([...paths.build.statics, "tags", `${page}.json`], pageTags)
}
...
for (const tag of tagsList) {
    ...
    for (const itemName of coreItems) {
        const meta = await FS.load([...paths.build.statics, "items", itemName, "meta.json"])
    ...
    for (let page = 1; page <= Math.max(1, tagPages); page++) {
        ...
        await FS.write([...paths.build.statics, "tags", tag, `${page}.json`], pageItems)
    }
}

log.ok(`Built tags index: ${tagsList.length} tags`)

const gamesDirs = await FS.dir(paths.src.games)
const games = []

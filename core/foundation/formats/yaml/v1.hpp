 ...
 * @doc: YAML parsing infrastructure for core communication protocol and configuration support. Provides comprehensive YAML 1.2 parsing and generation capabilities with zero external dependencies, moved from node implementation to core infrastructure to support YAML-RPC protocol and core configuration processing. Maintains all YAML features including key-value pairs, nested mappings and sequences, all scalar types, comments, multi-line values, anchors and aliases, multi-document streams, proper indentation handling, and Unicode support.
 * @specification: Core YAML infrastructure with complete YAML 1.2 support for protocol communication
 * @scope: Core foundation format processing infrastructure
 * @timeline: 2025-07-11
...
public:
    ...
    akao::foundation::types::NodeValue toNodeValue() const;
    static std::shared_ptr<YamlNode> fromNodeValue(const akao::foundation::types::NodeValue& value);
};
...
/**
 * YAML processor utilities for core infrastructure
 */
class YamlProcessor {
public:
    // Static utility methods for direct YAML processing
    static akao::foundation::types::NodeValue parseYaml(const std::string& yaml_content);
    static akao::foundation::types::NodeValue parseYamlFile(const std::string& file_path);
    static std::string generateYaml(const akao::foundation::types::NodeValue& data);
    static bool validateYaml(const std::string& yaml_content, std::vector<std::string>& errors);
    static std::vector<akao::foundation::types::NodeValue> parseMultiDocumentYaml(const std::string& yaml_content);

private:
    // Conversion utilities
    static akao::foundation::types::NodeValue yamlNodeToNodeValue(const std::shared_ptr<YamlNode>& yamlNode);
    static std::shared_ptr<YamlNode> nodeValueToYamlNode(const akao::foundation::types::NodeValue& nodeValue);
};

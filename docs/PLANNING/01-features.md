...
### 1. Tạo ví escrow cho seller (VSE Wallet Derivation)

**Vấn đề hiện tại:** Chưa có cách nào tạo ví trung gian giữ tiền cho seller. Không có ví escrow = không có sản phẩm.

**Cần làm:** Viết module tạo ví escrow dựa trên thuật toán trong white-paper (Bước 3):
- Input: public key của seller (xpub), secret giữa buyer và platform, mã đơn hàng (order id)
- Thuật toán: `seed = hash(buyer_platform_secret + order_id)` → dùng BIP-32 derive ra địa chỉ ví mới từ seller xpub + seed
- Output: địa chỉ ví VSE (dạng watch-only — ai cũng thấy nhưng chỉ platform có private key để rút)

**Output cụ thể:** File `src/core/Escrow.js` với function `deriveVSE(sellerXpub, buyerSecret, orderId)` → trả về `{ address, privateKey }`

**Giúp gì cho gọi vốn:** Đây là phần chứng minh công nghệ lõi — ví escrow tạo ra on-chain, kiểm chứng được trên blockchain explorer. Nhà đầu tư thấy tiền thật sự nằm trong ví mà không bên nào tự ý rút được.

**Tham khảo:** `docs/thoughts/white-paper-draft.md` — Bước 3, `src/core/Wallet.js` — pattern derive address hiện tại

---

### 2. Tạo ví escrow cho affiliate (VAE Wallet Derivation)
...
**Cần làm:** Mở rộng module Escrow để tạo ví VAE theo white-paper (Bước 3A):
- Input: public key của affiliate (xpub), secret giữa buyer và platform
- Thuật toán: tương tự VSE nhưng dùng affiliate xpub thay vì seller xpub
- Output: địa chỉ ví VAE (platform toàn quyền, affiliate nhận seed để unlock sau khi đơn hàng thành công)

**Output cụ thể:** Function `deriveVAE(affiliateXpub, buyerSecret)` trong `src/core/Escrow.js` → trả về `{ address, privateKey }`
...
- Tạo và phát 3 giao dịch on-chain:
  1. Tiền hàng → gửi vào ví VSE (escrow cho seller)
...
**Giúp gì cho gọi vốn:** Đây là **demo quan trọng nhất**. Nhà đầu tư bấm "Thanh toán", thấy 3 giao dịch on-chain, thấy tiền vào ví escrow. Chứng minh sản phẩm hoạt động thật, không phải mockup.
...
**Giúp gì cho gọi vốn:** Nhà đầu tư thấy đơn hàng với trạng thái "Đang giữ" → chứng minh tiền đang nằm trong escrow chờ xác nhận.
...
Order {
  ...
  vseAddress:   string    — địa chỉ ví escrow seller
  vaeAddress:   string    — địa chỉ ví escrow affiliate (null nếu không có)
...
**Vấn đề hiện tại:** Chưa có cách nào để tiền từ ví escrow VSE đến tay seller. Đây là bước cuối cùng hoàn tất vòng đời đơn hàng.
...
**Giúp gì cho gọi vốn:** Chứng minh toàn bộ flow tiền: buyer trả → escrow giữ → seller nhận. Vòng đời hoàn chỉnh. Nhà đầu tư thấy tiền thật sự di chuyển trên blockchain, không phải số trong database.

...
**Output cụ thể:** File `src/core/Platform.js` với function `deriveVSE(sellerXpub, buyerSecret, orderId)` → trả về `{ address, privateKey }`
...
**Output cụ thể:** Function `deriveVAE(affiliateXpub, buyerSecret)` trong `src/core/Platform.js` → trả về `{ address, privateKey }`

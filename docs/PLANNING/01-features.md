...
- Input: **`pub/epub` của seller**, secret giữa buyer và platform, mã đơn hàng (order id)
- Thuật toán: `unlockSeed = hash(buyer_platform_secret + order_id)` → dùng **ZEN secp256k1 derivation** derive ra public lock key từ `pub/epub` của seller
...
- Input: **`pub/epub` của affiliate**, secret giữa buyer và platform

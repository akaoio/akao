...
export class Logic {
    ...
    static loadfollowing(callback) {
        const pair = Access.get("pair")
        if (!pair) return null
        const scope = globalThis.gun.get(`~${pair.pub}`).get("following").map()
        scope.on(callback)
        return scope
    }
    ...
    static addfollow(pub, name, list) {
        ...
        const pair = Access.get("pair")
        if (!pair) return null
        globalThis.gun.get(`~${pair.pub}`).get("following").get(pub).put({ name, at: Date.now() }, null, { opt: { authenticator: pair } })
        return { pub, name }
    }
    ...
    static removefollow(pub) {
        const pair = Access.get("pair")
        if (!pair) return
        globalThis.gun.get(`~${pair.pub}`).get("following").get(pub).put(null, null, { opt: { authenticator: pair } })
    }
}

...
Test.describe("HashInfoError", () => {

    Test.it("has correct name, code, and message", () => {
        const err = new HashInfoError("INVALID_INPUT", "bad input")
        Test.assert.equal(err.name, "HashInfoError")
        Test.assert.equal(err.code, "INVALID_INPUT")
        Test.assert.equal(err.message, "bad input")
    })

    Test.it("is an instance of Error and HashInfoError", () => {
        const err = new HashInfoError("SOME_CODE", "msg")
        Test.assert.truthy(err instanceof Error)
        Test.assert.truthy(err instanceof HashInfoError)
    })

    Test.it("supports different error codes", () => {
        const codes = ["INVALID_INPUT", "CRYPTO_UNAVAILABLE", "INVALID_PATH", "STREAM_FAILED"]
        for (const code of codes) {
            const err = new HashInfoError(code, "msg")
...
Test.describe("hashInfo — Uint8Array input", () => {

    Test.it("returns v1 as 40-char hex string", async () => {
        const result = await hashInfo(makeBytes(1024), "file.bin")
    ...
    Test.it("returns null v2 by default", async () => {
        const result = await hashInfo(makeBytes(512), "file.bin")
        Test.assert.equal(result.v2, null)
    })

    Test.it("is deterministic — same bytes produce same v1", async () => {
        const bytes = makeBytes(512, 0x42)
        const r1 = await hashInfo(bytes, "file.bin")
        const r2 = await hashInfo(bytes, "file.bin")
        Test.assert.equal(r1.v1, r2.v1)
    })

    Test.it("different content produces different v1", async () => {
        const r1 = await hashInfo(makeBytes(512, 0x01), "file.bin")
        const r2 = await hashInfo(makeBytes(512, 0x02), "file.bin")
        Test.assert.notEqual(r1.v1, r2.v1)
    })

    Test.it("single byte input is accepted", async () => {
        const result = await hashInfo(new Uint8Array([0xFF]), "tiny.bin")
        Test.assert.equal(result.v1.length, 40)
    })
...
Test.describe("hashInfo — ArrayBuffer input", () => {

    Test.it("returns v1 for ArrayBuffer", async () => {
        const buf = makeBytes(256, 0x55).buffer
        const result = await hashInfo(buf, "data.bin")
        Test.assert.equal(result.v1.length, 40)
        Test.assert.equal(result.v2, null)
    })
    ...
    Test.it("produces same v1 as equivalent Uint8Array", async () => {
        ...
        const r1 = await hashInfo(bytes, "f.bin")
        const r2 = await hashInfo(bytes.buffer, "f.bin")
        Test.assert.equal(r1.v1, r2.v1)
    })
...
Test.describe("hashInfo — AsyncIterable input", () => {

    Test.it("accepts async generator and returns v1", async () => {
        const data = makeBytes(2048, 0x33)
        const result = await hashInfo(asyncChunks(data), "stream.bin", { length: data.length })
        Test.assert.equal(result.v1.length, 40)
        Test.assert.equal(result.v2, null)
    })
    ...
    Test.it("async iterable matches Uint8Array hash for same data", async () => {
        ...
        const r1 = await hashInfo(data, "file.bin")
        const r2 = await hashInfo(asyncChunks(data, 64), "file.bin", { length: data.length })
        Test.assert.equal(r1.v1, r2.v1)
    })

    Test.it("chunk size does not affect the hash", async () => {
        const data = makeBytes(1024, 0x55)
        const r1 = await hashInfo(asyncChunks(data, 32), "file.bin", { length: data.length })
        const r2 = await hashInfo(asyncChunks(data, 256), "file.bin", { length: data.length })
        Test.assert.equal(r1.v1, r2.v1)
    })
...
Test.describe("hashInfo — v2 option", () => {

    Test.it("returns v2 as 64-char hex string when v2:true", async () => {
        const result = await hashInfo(makeBytes(512, 0xAA), "file.bin", { v2: true })
    ...
    Test.it("v1 in hybrid mode differs from v1-only for same content", async () => {
        ...
        const v1only = await hashInfo(bytes, "file.bin")
        const hybrid = await hashInfo(bytes, "file.bin", { v2: true })
        Test.assert.notEqual(v1only.v1, hybrid.v1)
    })

    Test.it("v2 hash is deterministic", async () => {
        const bytes = makeBytes(1024, 0xCC)
        const r1 = await hashInfo(bytes, "file.bin", { v2: true })
        const r2 = await hashInfo(bytes, "file.bin", { v2: true })
        Test.assert.equal(r1.v2, r2.v2)
    })

    Test.it("different content produces different v2", async () => {
        const r1 = await hashInfo(makeBytes(512, 0x01), "f.bin", { v2: true })
        const r2 = await hashInfo(makeBytes(512, 0x02), "f.bin", { v2: true })
...
Test.describe("hashInfo — multi-file array", () => {
    ...
    Test.it("accepts array of file descriptors and returns v1", async () => {
        ...
        const result = await hashInfo(files, "myTorrent")
        Test.assert.equal(result.v1.length, 40)
        Test.assert.equal(result.v2, null)
    })
    ...
    Test.it("multi-file v1 is deterministic regardless of input order", async () => {
        ...
        const r1 = await hashInfo(filesAB, "myTorrent")
        const r2 = await hashInfo(filesBA, "myTorrent")
        Test.assert.equal(r1.v1, r2.v1)
    })
    ...
    Test.it("zero-byte files are included without being streamed", async () => {
        ...
        const result = await hashInfo(files, "mixed")
        Test.assert.equal(result.v1.length, 40)
    })
    ...
    Test.it("multi-file v2 returns both v1 and v2", async () => {
        ...
        const result = await hashInfo(files, "hybrid", { v2: true })
        Test.assert.equal(result.v1.length, 40)
        Test.assert.equal(result.v2.length, 64)
    })
...
Test.describe("hashInfo — options", () => {

    Test.it("private option changes the v1 hash", async () => {
        const bytes = makeBytes(256, 0x11)
        const r1 = await hashInfo(bytes, "file.bin")
        const r2 = await hashInfo(bytes, "file.bin", { private: true })
        Test.assert.notEqual(r1.v1, r2.v1)
    })
    ...
    Test.it("source option changes the v1 hash", async () => {
        ...
        const r1 = await hashInfo(bytes, "file.bin")
        const r2 = await hashInfo(bytes, "file.bin", { source: "mytracker" })
        Test.assert.notEqual(r1.v1, r2.v1)
    })

    Test.it("different pieceLength produces different v1", async () => {
        const bytes = makeBytes(16384, 0x33)
        const r1 = await hashInfo(bytes, "file.bin", { pieceLength: 16384 })
        const r2 = await hashInfo(bytes, "file.bin", { pieceLength: 32768 })
        Test.assert.notEqual(r1.v1, r2.v1)
    })
    ...
    Test.it("same options always produce same hash", async () => {
        ...
        const r1 = await hashInfo(bytes, "file.bin", opts)
        const r2 = await hashInfo(bytes, "file.bin", opts)
        Test.assert.equal(r1.v1, r2.v1)
    })
...
Test.describe("hashInfo — error handling", () => {

    Test.it("throws HashInfoError UNSUPPORTED_FORMAT for a number", async () => {
        const err = await Test.assert.rejects(hashInfo(12345, "file.bin"))
        Test.assert.truthy(err instanceof HashInfoError)
        Test.assert.equal(err.code, "UNSUPPORTED_FORMAT")
    })

    Test.it("throws HashInfoError UNSUPPORTED_FORMAT for null", async () => {
        const err = await Test.assert.rejects(hashInfo(null, "file.bin"))
        Test.assert.truthy(err instanceof HashInfoError)
        Test.assert.equal(err.code, "UNSUPPORTED_FORMAT")
    })

    Test.it("throws HashInfoError INVALID_INPUT for empty files array", async () => {
        const err = await Test.assert.rejects(hashInfo([], "name"))
        Test.assert.truthy(err instanceof HashInfoError)
        Test.assert.equal(err.code, "INVALID_INPUT")
    })

    Test.it("throws HashInfoError INVALID_PATH for path traversal (..)", async () => {
        const bytes = makeBytes(64)
        const files = [{ path: "dir/../etc/passwd", length: bytes.length, stream: makeStreamFactory(bytes) }]
        const err = await Test.assert.rejects(hashInfo(files, "name"))
        Test.assert.truthy(err instanceof HashInfoError)
        Test.assert.equal(err.code, "INVALID_PATH")
    })

    Test.it("throws HashInfoError INVALID_PIECE_LENGTH for non-power-of-two", async () => {
        const err = await Test.assert.rejects(hashInfo(makeBytes(256), "file.bin", { pieceLength: 20000 }))
        Test.assert.truthy(err instanceof HashInfoError)
        Test.assert.equal(err.code, "INVALID_PIECE_LENGTH")
    })

    Test.it("throws HashInfoError INVALID_PIECE_LENGTH for value below 16384", async () => {
        const err = await Test.assert.rejects(hashInfo(makeBytes(256), "file.bin", { pieceLength: 8192 }))
        Test.assert.truthy(err instanceof HashInfoError)
        Test.assert.equal(err.code, "INVALID_PIECE_LENGTH")
    })

    Test.it("throws HashInfoError STREAM_LENGTH_MISMATCH when declared length differs from actual", async () => {
    ...
    Test.it("throws HashInfoError INVALID_INPUT for negative file length", async () => {
